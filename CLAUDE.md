...
**Hash-locked pip lockfiles (compiled from `pyproject.toml`/`uv`, no `requirements.in`)**: Dependabot edits such lockfiles entry-by-entry without re-resolving, so when a package exact-pins another (e.g. `pydantic` pins `pydantic-core==<version>`) it bumps one of the pair alone and produces an uninstallable lockfile. `allow: dependency-type: direct` does **not** help — a flat fully-pinned lockfile lists everything explicitly, so Dependabot treats all entries as "direct" and the filter is a no-op (confirmed empirically). The fix: `ignore` **both** names of each exact-pinned pair on every pip entry so it moves only via a periodic `--upgrade` recompile; add a pair only when a PR actually fails at `pip install`. Transitive CVEs still fail CI via `pip-audit`. Full rationale + backlog-consolidation technique: `docs/supply-chain.md` (Dependabot on hash-locked lockfiles).

**Auto-merging Dependabot PRs**: the merge must be gated on CI success — never a bare `gh pr merge --squash`, which merges immediately and can land a red build on `main`. There are two ways to achieve the gate, and **which one you must use depends on the repo's plan/visibility**. In a hash-locked repo, also ensure the generated artifacts (SBOM, and lockfile hashes if transitively affected) are refreshed on each bump — Dependabot does not do this itself. See `docs/supply-chain.md` (Dependabot and generated artifacts).

- **Option A — required status check + `gh pr merge --auto` (the textbook approach, but it has a precondition).** Enable branch protection on the default branch requiring the CI check, then `gh pr merge --auto` waits for that required check before merging. **This only works where branch protection is available.** It is **not** available on a **private repo on the GitHub Free plan** — the branch-protection / ruleset API returns `403: "Upgrade to GitHub Pro or make this repository public"`. On such a repo `--auto` has no required check to wait on, so it silently degrades to an **immediate** merge the instant the PR is approved + mergeable, *before CI finishes*. Confirmed the hard way: a `--auto` merge landed a Dependabot PR mid-CI-run, committing a stale-SBOM (red) build to `main` (specie77/trading-system PR #430). **Do not rely on `--auto` as a CI gate unless you have verified branch protection is actually enforceable on that repo.** `--auto` is a convenience, not the gate — the required check is the gate.

- **Option B — `workflow_run`-triggered merge (works with no branch protection; use this on Free-plan private repos).** Split auto-merge into two workflows so the CI result becomes a precondition *by construction* rather than by a branch-protection rule:
  1. A `pull_request_target` workflow runs `dependabot/fetch-metadata` (which **requires a PR event context** — it errors in a `workflow_run` job, which is the reason for the split) + `pip-audit`, and for patch/minor bumps only **approves** the PR. It does **not** merge.
  2. A second workflow triggered on `workflow_run` of the CI workflow's `completed` event does the **merge**, gated on `github.event.workflow_run.conclusion == 'success'`, a `dependabot/` head-branch prefix, and the presence of the auto-approval from workflow 1. Because this job only exists once CI has already concluded, a green result is structurally required — no branch protection needed. The approving review is the durable cross-workflow signal linking the two (approvals survive the SBOM-refresh bot's fix-commit push because Free-plan repos have no "dismiss stale approvals" rule to strip it). Gate on the branch prefix, not the actor, so a green run *re-triggered by the SBOM bot's push* still merges. `workflow_run` jobs always run from the default branch's copy of the workflow, so PR content can't alter the merge logic. Reference implementation: specie77/trading-system `.github/workflows/dependabot-approve.yml` + `dependabot-merge.yml` (PR #449).

- **`@dependabot recreate` does not work on hash-locked lockfiles** — it returns *"Dependabot can't resolve your Python dependency files."* Dependabot can open a bump by editing lockfile entries, but `recreate` forces a full re-resolution it cannot perform on a `--generate-hashes` lockfile. **Consequence:** you cannot retro-fix an *already-open* stale Dependabot PR (e.g. one opened before the SBOM/auto-merge pipeline was wired) by recreating it. Close those PRs and let the next **scheduled** Dependabot run reopen them fresh through the now-fixed pipeline, or do the manual consolidation in `docs/supply-chain.md` ("Consolidating a Dependabot backlog"). Leave a comment on each closed PR recording this, so a later scheduled run hitting the same wall has the plan on hand.
