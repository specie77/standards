...
**Hash-locked pip lockfiles (compiled from `pyproject.toml`/`uv`, no `requirements.in`)**: restrict each pip update entry to `allow: [{dependency-type: "direct"}]`. Dependabot edits such lockfiles entry-by-entry without re-resolving, so unrestricted it bumps a transitive past its pinned parent and produces an uninstallable lockfile (e.g. `pydantic-core` ahead of what `pydantic` pins). Transitive freshness is then handled by a periodic `--upgrade` recompile; transitive CVEs still fail CI via `pip-audit`. Full rationale + backlog-consolidation technique: `docs/supply-chain.md` (Restrict Dependabot to direct dependencies).
